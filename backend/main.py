...
async def create_task(project_id: str, task: TaskCreate):
    task_doc = {
        ...
        "created_at": datetime.now(timezone.utc),
        "assignee_id": ObjectId(task.assignee_id) if task.assignee_id else None
    }
...
async def create_notification(user_id: str, message: str, created_by: str = None):
    notif_doc = {
        "recipient_id": ObjectId(user_id),   # required
        "title": "Task Assigned",            # required
        "type": "task_assignment",           # required
...
def serialize_notification(notification: dict) -> dict:
    return {
        ...
        "read_at": notification["read_at"].isoformat() if isinstance(notification.get("read_at"), datetime) else notification.get("read_at"),
        "created_by": str(notification["created_by"]) if "created_by" in notification else None
    }
...
async def mark_as_read(notification_id: str, current_user: dict = Depends(get_current_user)):
    ...
    # Check ownership
    if str(notif["recipient_id"]) != str(current_user["_id"]):
        raise HTTPException(status_code=403, detail="Access denied")
